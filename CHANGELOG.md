...
### Fixed
- **aapt2 link errors** caused by `styles.xml` and `attrs.xml` in resource-only packages
  - These files are now automatically ignored during compilation
  - Prevents errors when custom attributes or styles reference non-existent resources
  - Includes debug logging to show when files are being skipped
...
- **Multi-module project support**
  - Build multiple modules independently
  - Merge module packages into a single file
  - New `build-multi` command
