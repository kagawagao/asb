...
- Unified skin package with all module resources
- Parallel resource compilation
...
For the full-android-app example, which demonstrates including multiple module resources:

```bash
cd full-android-app

# Build unified skin package with all module resources
../../target/release/asb build --config asb.config.json

# This creates:
# - build/com.example.skinapp.app.skin (containing all module resources)
```
...
3. Create an `AndroidManifest.xml` (or let asb auto-generate it)
...
2. Use `additionalResourceDirs` in config to include additional module resources
3. Build with `asb build --config asb.config.json`

For multiple independent apps:

1. Create separate directories for each app
2. Use `apps` array in config with shared settings at top level
3. Build all apps with single command: `asb build --config asb.config.json`

## Notes

- All examples assume `ANDROID_HOME` is set in your environment
- The examples use Android API 34, but you can use any API level
- The output skin files are resource-only packages suitable for hot updates
- The full-android-app example demonstrates how to include resources from multiple modules
- The array-config example shows how to build multiple independent apps with one command
